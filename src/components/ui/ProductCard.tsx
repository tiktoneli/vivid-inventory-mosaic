...
const ProductCard = ({
  ...
  return (
    <div className={`bg-card rounded-lg border overflow-hidden shadow-sm hover:shadow transition-shadow duration-300 ${!isActive ? 'border-red-300 bg-red-50' : 'border-border'}`}>
      <div className="p-4">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center">
              <p className="text-sm font-medium line-clamp-1 text-[#445372]">{name}</p>
              {!isActive && (
                <span className="ml-2 bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full">Inactive</span>
              )}
            </div>
          ...
          <div className="relative group">
            <button className="p-1 text-muted-foreground hover:text-[#00859e] rounded-full transition-colors">
        ...
        <div className="mt-3 flex flex-col gap-1">
          ...
          <div className="flex justify-between items-center">
            <span className={`text-xs font-medium ${isLowStock ? 'text-destructive' : 'text-[#00859e]'}`}>
              Stock: {stock} {isLowStock && <AlertCircle className="inline h-3 w-3 ml-1" />}
            </span>
            <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${
              isLowStock 
                ? 'bg-destructive/10 text-destructive' 
                : 'bg-[#00859e]/10 text-[#00859e]'
            }`}>
